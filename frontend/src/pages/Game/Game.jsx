...
function Game() {
  ...
  const [state, setState] = useState({
    mySessionId: 'SessionSADSssssssF',
  ...
  const OV = new OpenVidu(); // Change const to let
  ...
  function handleMainVideoStream(streamManager) {
  ...
  useEffect(() => {
    ...
    mySession.on('streamDestroyed', event => {
      ...
      console.log('삭제된 스트림', event.stream);
      setState(prevState => ({
        ...prevState,
    ...
    window.addEventListener('unload', () => state.session.disconnect);
  ...
  function handleButton() {
    state.subscribers.forEach((sub, idx) => console.log(idx, sub.stream));
  }
  ...
  return (
    <div>
      <button onClick={handleButton}>구독 세션 확인</button>
      <div
        style={{
          display: 'flex',
      ...
      {state.publisher !== undefined ? (
        <div className="stream-container col-md-3 col-xs-3 col-lg-3">
          <div className="streamcontainer">
            <button onClick={handleLeaveSession}>나가기</button>
            <button onClick={toggleAudio}>{audioEnabled ? '마이크 끄기' : '마이크 켜기'}</button>
